...
## 1) 세션 스냅샷
- 마지막 업데이트: 2026-03-13 (KST)
- 현재 단계: 포스팅 자동화 + Admin Post UI 구현 완료
...
- [x] 포스팅 자동화 + Admin Post UI:
  - `.claude/settings.json` — 공용 권한 자동승인 (git tracked)
  - `.claude/commands/post.md` — `/post <url>` 슬래시 커맨드
  - `AdminBar.tsx` — Post 탭 추가
  - `src/app/admin/post/page.tsx` — PostPage 클라이언트 컴포넌트
  - `src/app/[lang]/admin/post/page.tsx` — wrapper
  - `src/app/api/admin/post/route.ts` — workflow_dispatch 트리거
  - `src/app/api/admin/post/status/route.ts` — 상태 폴링
  - `.github/workflows/create-post.yml` — GitHub Actions 워크플로우

## 4) 진행 중 / 막힘
- **필수 설정**: `ANTHROPIC_API_KEY`를 GitHub repo Settings → Secrets → Actions에 추가해야 워크플로우 동작

## 5) 다음 3개 작업 (우선순위)
1. GitHub repo에 `ANTHROPIC_API_KEY` Secret 추가
2. Admin UI에서 arXiv URL로 테스트 포스팅 검증
3. 기존 포스트 meta.json에 `source_date`, `display_tags` 추가
