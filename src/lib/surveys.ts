...
/**
 * Strip private URLs from group-visibility surveys so they don't leak
 * into client-rendered HTML / RSC payloads. Title / cover / TOC stay
 * public per product policy; only embed_url and external links are
 * redacted because those are the ones that bypass the auth gate.
 */
function sanitizeGroupSurvey(s: SurveyMeta): SurveyMeta {
  if (s.visibility !== 'group') return s;
  return { ...s, embed_url: undefined, links: [] };
}

/**
 * Public listing view: surveys.json entries with private URLs stripped
 * for group-restricted items. Safe to return to any caller, any runtime.
 */
export async function loadPublicSurveys(): Promise<SurveyMeta[]> {
  return loadSurveysRaw().map(sanitizeGroupSurvey);
}
 ...
 * for group surveys. The caller (survey detail page) is responsible for
 * running the group auth gate before passing embed_url to the client
 * iframe component.
 */
